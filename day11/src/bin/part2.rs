...
impl Operation {
    fn apply(&self, old: &BigInt) -> BigInt {
        match self {
...
impl Operand {
    fn apply<'a>(&'a self, old: &'a BigInt) -> &'a BigInt {
        match self {
...
impl Condition {
    fn action(&self, value: &BigInt) -> &Action {
...
impl Test {
    fn passes(&self, value: &BigInt) -> bool {
        match self {
            Test::DivisibleBy(divisor) => (value % divisor) == BigInt::zero(),
        }
    }
}
